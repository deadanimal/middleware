...
export class LoginComponent implements OnInit {
  ...
  login() {
    ...
    else if (this.loginForm.value.username == 'user@prototype.com.my') {
      this.authService.userRole = 2
      this.navigatePage('dashboard-user')
    }
    else if (this.loginForm.value.username == 'organisation@prototype.com.my') {
      this.authService.userRole = 3
      this.navigatePage('dashboard-organisation')
    }
  }
  ...
  navigatePage(path: String) {
    ...
    else if (path == 'dashboard-organisation') {
      return this.router.navigate(['/organisation/dashboard'])
    }
  }
